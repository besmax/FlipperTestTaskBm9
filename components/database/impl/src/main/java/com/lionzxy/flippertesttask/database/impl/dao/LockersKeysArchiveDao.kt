...
interface LockersKeysArchiveDao {
    ...
    @Query("""SELECT locker_id AS number, key_id AS key_number FROM lockers_keys_archive_table""")
    ...
    @Query("SELECT lockers_keys_archive_table.id, lockers_keys_archive_table.locker_id, lockers_keys_archive_table.key_id FROM lockers_table " +
            ...
            "WHERE lockers_keys_archive_table.locker_id = :lockerNumber")
    suspend fun get(lockerNumber: Int): LockersKeysArchiveEntity?
